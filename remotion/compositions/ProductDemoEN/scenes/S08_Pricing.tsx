/**
 * S08 Pricing — Plan Cards (English)
 */
    ...
    <SceneContainer>
      ...
      <CenteredContent style={{ opacity: exitOpacity }}>
        <div style={{ marginBottom: 50 }}>
          <h2 style={{ fontFamily: FONTS.display, fontSize: 48, fontWeight: 700, color: COLORS.textPrimary, margin: 0, opacity: titleOpacity }}>
            Plans & Pricing
          </h2>
        ...
        <div style={{ display: 'flex', gap: 32, alignItems: 'stretch', width: '100%', justifyContent: 'center' }}>
          {PRICING_PLANS_EN.map((plan, i) => {
            ...
            return (
              <div key={i} style={{
                flex: 1, maxWidth: 380, opacity: cardProgress,
                ...
                <GlassCard highlighted={plan.highlighted} style={{ padding: '40px 32px', height: '100%', display: 'flex', flexDirection: 'column' }}>
                  <h3 style={{ fontFamily: FONTS.display, fontSize: 24, fontWeight: 700, color: plan.highlighted ? COLORS.primary : COLORS.textPrimary, marginBottom: 8 }}>
                    {plan.name}
                  </h3>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 24, paddingBottom: 24, borderBottom: `1px solid ${COLORS.borderColor}` }}>
                    <span style={{ fontFamily: FONTS.display, fontSize: 48, fontWeight: 900, color: COLORS.textPrimary }}>
                      {plan.currency}{plan.price}
                    </span>
                    <span style={{ fontSize: 16, color: COLORS.textMuted }}>{plan.period}</span>
                  </div>

                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14, flex: 1 }}>
                    {plan.features.map((feature, fi) => {
                      const featureOpacity = fadeIn(frame, delay + 40 + staggerDelay(fi, 8), 12);
                      return (
                        <div key={fi} style={{ display: 'flex', alignItems: 'center', gap: 10, opacity: featureOpacity }}>
                          <CheckIcon size={18} color={plan.highlighted ? COLORS.primary : COLORS.teal} />
                          <span style={{ fontSize: 15, color: COLORS.textSecondary }}>{feature}</span>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <div style={{
                    marginTop: 28, padding: '14px 24px', borderRadius: 10, textAlign: 'center', fontWeight: 600, fontSize: 16,
        ...
        <p style={{ marginTop: 40, fontSize: 18, color: COLORS.textSecondary, opacity: trialOpacity, fontWeight: 500 }}>
          ✨ All plans include a{' '}
