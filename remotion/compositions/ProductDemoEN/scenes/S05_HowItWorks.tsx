/**
 * S05 How It Works — 3-Step Flow (English)
 */
          ...
          {STEPS_EN.map((step, i) => {
            ...
            return (
              <React.Fragment key={i}>
                <GlassCard
                  style={{
                    width: 340, textAlign: 'center', opacity: cardProgress,
                    transform: `translateY(${(1 - cardProgress) * 40}px)`, position: 'relative',
                  }}
                >
                  <div style={{
                    ...
                    margin: '0 auto 20px', boxShadow: `0 0 20px ${COLORS.glowTeal}`,
                  }}>
                    <span style={{ fontFamily: FONTS.display, fontSize: 24, fontWeight: 800, color: '#fff' }}>{step.number}</span>
                  </div>
                  <h3 style={{ fontFamily: FONTS.display, fontSize: 24, fontWeight: 700, color: COLORS.textPrimary, marginBottom: 10 }}>
                    {step.title}
                  </h3>
                ...
                {i < STEPS_EN.length - 1 && (
                  <div style={{ width: 80, height: 2, position: 'relative', display: 'flex', alignItems: 'center' }}>
                    <div style={{
                      width: `${(i === 0 ? line1Progress : line2Progress) * 100}%`, height: 2,
                      background: `linear-gradient(90deg, ${COLORS.teal}, ${COLORS.tealLight})`, borderRadius: 1,
                    }} />
                    <div style={{
