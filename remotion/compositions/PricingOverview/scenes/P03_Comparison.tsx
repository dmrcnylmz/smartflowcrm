/**
 * P03 Comparison — Feature Highlights (6s / 180 frames)
 *
 * Key differentiators shown as comparison rows
 */
...
const COMPARISON_ROWS = [
  { feature: 'AI Sesli Asistan', starter: true, pro: true, enterprise: true },
  { feature: 'Otomatik Randevu', starter: true, pro: true, enterprise: true },
  { feature: 'CRM Entegrasyonu', starter: false, pro: true, enterprise: true },
  { feature: 'Gelişmiş Analitik', starter: false, pro: true, enterprise: true },
  { feature: 'Özel Model Eğitimi', starter: false, pro: false, enterprise: true },
  { feature: 'API Erişimi', starter: false, pro: false, enterprise: true },
  { feature: 'SLA Garantisi', starter: false, pro: false, enterprise: true },
];
    ...
    <SceneContainer>
      ...
      <CenteredContent style={{ opacity: exitOpacity, maxWidth: 1200 }}>
        ...
        >
          ...
          >
            <div style={{ fontSize: 16, color: COLORS.textMuted, fontWeight: 500 }}>Özellik</div>
            <div style={{ fontSize: 16, color: COLORS.textSecondary, fontWeight: 600, textAlign: 'center' }}>Başlangıç</div>
            <div style={{ fontSize: 16, color: COLORS.primary, fontWeight: 700, textAlign: 'center' }}>Profesyonel</div>
            <div style={{ fontSize: 16, color: COLORS.textSecondary, fontWeight: 600, textAlign: 'center' }}>Kurumsal</div>
          </div>

          {/* Feature rows */}
          {COMPARISON_ROWS.map((row, i) => {
            const rowDelay = 30 + staggerDelay(i, 8);
            const rowProgress = springIn({
              fps,
              frame,
              delay: rowDelay,
              config: { damping: 20, mass: 0.3, stiffness: 120 },
            });
