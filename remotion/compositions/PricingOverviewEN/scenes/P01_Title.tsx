/**
 * P01 Title — Pricing Overview Title (English)
 */
...
import { fadeIn, fadeOut, scaleIn, drawLine, slideUp } from '../../../utils/animations';
  ...
  return (
    <SceneContainer>
      ...
      <CenteredContent style={{ opacity: exitOpacity }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 40, transform: `scale(${logoScale})` }}>
          <div style={{
          ...
          <span style={{ fontFamily: FONTS.display, fontSize: 30, fontWeight: 800, color: COLORS.textPrimary, letterSpacing: 4 }}>CALLCEPTION</span>
        </div>

        <div style={{ opacity: titleOpacity, transform: `translateY(${titleSlide}px)`, marginBottom: 16 }}>
          <GradientText fontSize={64} fontWeight={800}>Plans & Pricing</GradientText>
        </div>
        ...
        <p style={{ fontSize: 22, color: COLORS.textSecondary, opacity: subtitleOpacity, fontWeight: 400 }}>
          AI-powered call management for businesses of every size
        </p>
      </CenteredContent>
    </SceneContainer>
  );
